...
func LoginNutritionist(c *gin.Context) {
	...
	// ค้นหา nutritionist ด้วย email ที่ผู้ใช้กรอกเข้ามา
	if err := entity.DB().Raw("SELECT * FROM nutritionists WHERE email = ?", payload.Email).Scan(&nutritionist).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	jwtWrapper := service.JwtWrapper{
		SecretKey:       "SvNQpBN8y3qlVrsGAYYWoJJk56LtzFHx",
		Issuer:          "AuthService",
		ExpirationHours: 24, // ระยะเวลาหมดของ token
	}

	// สร้าง token
	signedToken, err := jwtWrapper.GenerateToken(nutritionist.Email)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "error signing token"})
		return
	}
